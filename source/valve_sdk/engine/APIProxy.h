...
#include "archtypes.h"
#include "valve_sdk\common\netadr.h"
#include "valve_sdk\common\Sequence.h"
...
typedef struct
{
	...
	DST_HUD_CHATINPUTPOSITION_FUNC			pChatInputPosition;
	DST_HUD_GETPLAYERTEAM					pGetPlayerTeam;
} cldll_func_dst_t;
...
typedef struct engdata_s
{
	...
	cldll_func_dst_t *pg_cldstAddrs;
	struct modfuncs_s *pg_modfuncs;
	struct cmd_function_s **pcmd_functions;
	void *pkeybindings;
	void (*pfnConPrintf)(char *, ...);
	struct cvar_s **pcvar_vars;
	struct glwstate_t *pglwstate;
	void *(*pfnSZ_GetSpace)(struct sizebuf_s *, int);
	struct modfuncs_s *pmodfuncs;
	void *pfnGetProcAddress;
	void *pfnGetModuleHandle;
	struct server_static_s *psvs;
	struct client_static_s *pcls;
	void (*pfnSV_DropClient)(struct client_s *, qboolean, char *, ...);
	void (*pfnNetchan_Transmit)(struct netchan_s *, int, byte *);
	void (*pfnNET_SendPacket)(enum netsrc_s sock, int length, void *data, netadr_t to);
	struct cvar_s *(*pfnCvarFindVar)(const char *pchName);
...
typedef struct modfuncs_s
{
	...
	PFN_MODULELOADED m_pfnModuleLoaded;

	PFN_PROCESSOUTGOINGNET m_pfnProcessOutgoingNet;
	PFN_PROCESSINCOMINGNET m_pfnProcessIncomingNet;

	PFN_TEXTURELOAD m_pfnTextureLoad;
	PFN_MODELLOAD m_pfnModelLoad;

	PFN_FRAMEBEGIN m_pfnFrameBegin;
	PFN_FRAMERENDER1 m_pfnFrameRender1;
	PFN_FRAMERENDER2 m_pfnFrameRender2;
	...
	PFN_CONNECTCLIENT m_pfnConnectClient;
	PFN_RECORDIP m_pfnRecordIP;
	PFN_PLAYERSTATUS m_pfnPlayerStatus;

	PFN_SETENGINEVERSION m_pfnSetEngineVersion;
...
#define k_nEngineVersion16Validated	3
...
#define k_nChecksumCompensator 0x36a8f09c
